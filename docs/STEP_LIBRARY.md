...
| `map` | object | yes | Per-record template. `external_id` and `title` are required; `content`, `excerpt`, `status`, `fields`, `taxonomies`, `featured_image_id`, `featured_image_url` optional. Only the keys present are written, so a field you do not map keeps its current value on the site. `featured_image_url` is a feed's photo URL: Post Runtime sideloads it into the media library once per URL (re-runs reuse the attachment) and sets it as the record's featured image; a URL that cannot be fetched is a per-record warning, and the record is still written. |
