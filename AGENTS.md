...
> **🤖 MCP connector (v0.5.0):** Codex Desktop bridge lives at `includes/Connectors/MCP/`:
...
>
> Setup flow for end users: WP admin → FlowMint Workflows → Codex Connection → toggle "Enable Codex Cowork Connection" → Generate Connection → copy the Terminal command, paste into macOS Terminal, restart Codex Desktop. Default state is **disabled**; `FMW_REST_Auth::require_manage()` checks both `manage_options` and the kill switch (with `/preflight` exempt so Codex can introspect "is this on?" without it being on). See `docs/MCP_CONNECTOR_SETUP.md` for full setup notes.
...
- A connector REST API + MCP tool layer so workflows can be created and managed by AI tools (Codex) the same way FormEngine forms are
...
| **Detailed patterns, examples, gotchas** | `docs/AGENTS.md` |
...
flowmint-workflows/
  ...
  AGENTS.md                    # This file — AI-facing reference
