...
class FMW_REST_Preflight {
    ...
    public function handle( $request ) {
        ...
        return rest_ensure_response( FMW_REST_Auth::success( [
            ...
            'context_shape' => [
                'description'  => 'Top-level namespaces accessible via {{ … }} interpolation in any string field of any step config. Use these paths everywhere a step takes a string (subject, body, message, to, url, headers, etc.). There is also env, which holds exactly {{ env.site_name }}, {{ env.site_url }} and {{ env.admin_email }} and nothing else: stored credentials cannot be read through {{ env.* }} (a missing path resolves to an empty string), so an API token for an http_* step has to be written into its headers.',
                'namespaces'   => [
                    ...
                    'labels'      => 'The SAME keys as data, but with select / radio / checkbox values resolved to the option labels the visitor actually saw. Use this ANY TIME a value reaches a person — email subjects and bodies, Slack messages, generated documents. {{ labels.workshop }} gives "Hand-Cut Joinery Intensive" where {{ data.workshop }} gives "joinery". Keys with no option list resolve to their raw value, so labels is always safe and never emptier than data. Keep using data for machine destinations (http_post, printavo_*), which want stable identifiers.',
                    ...
                    'entry_files' => 'A MAP KEYED BY FIELD KEY, not a list — {{ entry_files.reference_images.file_name }}, never {{ entry_files[0].file_name }}. Each value carries id, entry_id, field_key, attachment_id, file_path, file_name, file_size, mime_type, created_at.\n\nSHAPE SHIFTS WITH COUNT. For a multi-file field the value is a SINGLE OBJECT when one file was uploaded and an ARRAY OF OBJECTS when two or more were. A workflow written against one upload therefore breaks silently on the second, because the interpolator resolves a missing path to empty string rather than failing. Handle both shapes, or prefer the typed file steps (fre_get_file, drive_upload_file) which normalise this for you.\n\nNO file_url. Promptless Forms webhook payloads include a publicly-fetchable file_url, but its resolver is private to the webhook dispatcher and this context does not carry one. Use file_path for server-side work, or attachment_id with WordPress own URL helpers. Anything interpolating {{ …file_url }} here silently yields an empty string.',

                    'run'         => 'Current workflow run: { id, started_at }. Example: {{ run.id }} in a log message for traceability.',
                    'workflow'    => 'Workflow metadata: { id, title }. Example: {{ workflow.id }} when one log line covers many workflows.',
                    'form'        => 'Bound form metadata: { id, title } (empty for scheduled runs). Example: {{ form.id }} when the same listener handles multiple forms.',
